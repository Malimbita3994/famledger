...
class IncomeController extends Controller
{
    ...
    public function store(Request $request, Family $family)
    {
        ...
        broadcast(new FamilyFinancialDataChanged($family->id, 'income_created'));
    ...
    public function show(Income $income, Family $family): View
    {
        $this->authorizeFamilyMember($family);
        $this->assertIncomeInFamily($family, $income);
        ...
        $income->load([
            ...
            'createdBy:id,name',
            'receivedBy:id,name',
            'linkedProject:id,name',
            'linkedProperty:id,name',
            'familyLiability:id,name,type',
        ]);

        return view('families.incomes.show', compact('family', 'income'));
    }

    public function edit(Income $income, Family $family): View|RedirectResponse
    {
        $this->authorizeFamilyMember($family);
        $this->assertIncomeInFamily($family, $income);
        ...
        if (! $wallet || $wallet->status !== 'active' || $wallet->family_id !== $family->id) {
            return redirect()
                ->route('families.incomes.index')
                ->with('error', __('This income’s wallet is missing or inactive.'));
        }
        ...
        return view('families.incomes.edit', [
            'family' => $family,
            'mainWallet' => $wallet,
            'income' => $income->load('category.parent'),
            'categoryParents' => $categoryParents,
            'flatCategories' => $flatCategories,
            'incomeGroupToSubIds' => $incomeGroupToSubIds,
            'incomeCategoryIdToGroupSub' => $incomeCategoryIdToGroupSub,
    ...
    public function update(Request $request, Income $income, Family $family): RedirectResponse
    {
        $this->authorizeFamilyMember($family);
        $this->assertIncomeInFamily($family, $income);
        ...
        if (! $wallet || $wallet->status !== 'active') {
            return redirect()
                ->route('families.incomes.index')
                ->with('error', __('Wallet for this income is missing or inactive.'));
        }
        ...
        $validated = $request->validate([
            ...
            'amount.min' => 'Amount must be greater than zero.',
            'currency_code.in' => 'Currency must match the wallet.',
            'category_id.required' => 'Please choose a category for this income.',
            'recurring_frequency.required_if' => 'Choose how often this recurring income happens.',
        ]);
        ...
        broadcast(new FamilyFinancialDataChanged($family->id, 'income_updated'));
    ...
    public function destroy(Income $income, Family $family): RedirectResponse
    {
        $this->authorizeFamilyMember($family);
        $this->assertIncomeInFamily($family, $income);
        ...
        broadcast(new FamilyFinancialDataChanged($family->id, 'income_deleted'));

        return redirect()
            ->route('families.incomes.index')
            ->with('success', __('Income removed.'));
    }

    protected function assertIncomeInFamily(Family $family, Income $income): void
    {
        abort_unless($income->family_id === $family->id, 404);
    }
}
