    ...
    <div class="famledger-page-header">
        <div class="kt-container-fixed flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
                ...
                <h1 class="admin-pulse-title">{{ __('Create permission') }}</h1>
            ...
            <x-fin-back-link href="{{ route('admin.permissions.index') }}" class="!mb-0 shrink-0">{{ __('Back to permissions') }}</x-fin-back-link>
        </div>
    </div>

    <div class="kt-container-fixed px-4 sm:px-6 lg:px-8 pb-14">
        <form action="{{ route('admin.permissions.store') }}" method="POST" class="admin-pulse-create">
            @csrf

            <div class="admin-pulse-frame">
                <div class="admin-pulse-card-inner">
                    <h2 class="admin-pulse-section-title mb-6">{{ __('Permission details') }}</h2>

                    <div class="flex flex-col gap-1.5">
                        <label for="name" class="admin-pulse-field-label">
                            {{ __('Permission name') }} <span class="text-red-600 dark:text-red-400">*</span>
                        </label>
                        <input
                            type="text"
                            name="name"
                            ...
                            required
                            class="kt-input"
                            placeholder="{{ __('e.g. dashboard_view, reports_download') }}"
                            autocomplete="off"
                        />
                        <p class="admin-pulse-hint mt-0.5">{{ __('Lowercase recommended; use underscores. Must be unique in the system.') }}</p>
                        @error('name')
                    ...
                    <div class="flex flex-row flex-nowrap justify-end items-center gap-3 pt-8 mt-6 border-t border-sky-100/80 dark:border-slate-600/50">
                        <a href="{{ route('admin.permissions.index') }}" class="admin-pulse-btn-outline justify-center text-center shrink-0 whitespace-nowrap">{{ __('Cancel') }}</a>
