...
<div class="kt-container-fixed px-4 sm:px-6 lg:px-8 py-6 lg:py-8 pb-12">
    <a href="{{ route('families.incomes.index') }}" class="inline-flex items-center text-sm text-muted-foreground hover:text-foreground transition-colors mb-6">
        <i class="ki-filled ki-left text-base mr-1"></i>
        Back to income
    </a>

    <form action="{{ route('families.incomes.store') }}" method="POST" id="income-form">
        @csrf
            ...
            <div class="kt-card pb-2.5">
                ...
                <div class="kt-card-content grid gap-5">
                        ...
                        @else
                            <div class="famledger-form-row-3 gap-5 mb-5">
                                <div class="famledger-form-field grid gap-1.5 md:col-span-3">
                                    <label for="category_id" class="kt-form-label">{{ __('Category') }} <span class="text-destructive">*</span></label>
                                    <select name="category_id" id="category_id" class="kt-select w-full" required>
                                        <option value="">{{ __('Select category') }}</option>
                    ...
                    <div>
                        ...
                        <div class="famledger-form-row-3 gap-5 mb-5">
                            <div class="famledger-form-field grid gap-1.5">
                                ...
                                <div class="flex flex-col gap-2 pt-0.5">
                                    <label class="inline-flex items-center gap-2 text-sm cursor-pointer">
                                        <input type="checkbox" name="is_recurring" id="is_recurring" value="1" class="rounded border-input" {{ old('is_recurring') ? 'checked' : '' }} />
                            ...
                            <div class="famledger-form-field grid gap-1.5" id="js_recurring_frequency_wrap" style="display: none;">
                                ...
                                <select name="recurring_frequency" id="recurring_frequency" class="kt-select w-full">
                                    ...
                                    @foreach ($recurringFrequencies as $value => $label)
                                        <option value="{{ $value }}" @selected(old('recurring_frequency') === $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="famledger-form-field grid gap-1.5">
                                ...
                                <label class="inline-flex items-center gap-2 text-sm cursor-pointer pt-0.5">
                                    <input type="checkbox" name="is_taxable" id="is_taxable" value="1" class="rounded border-input" @checked(old('is_taxable', '1') === '1') />
                        ...
                        <div class="famledger-form-row-3 gap-5 mb-5">
                            <div class="famledger-form-field grid gap-1.5">
                                ...
                                <select name="source_entity_type" id="source_entity_type" class="kt-select w-full">
                                    ...
                                    @foreach ($sourceEntityTypes as $value => $label)
                                        <option value="{{ $value }}" @selected(old('source_entity_type') === $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="famledger-form-field grid gap-1.5 md:col-span-2">
                                ...
                                <input type="text" name="source" id="source" value="{{ old('source') }}" placeholder="{{ __('e.g. Acme Corp, Jane Doe, Unit 4B tenant') }}" class="kt-input" aria-invalid="{{ $errors->has('source') ? 'true' : 'false' }}" />
                            ...
                            <div class="famledger-form-field grid gap-1.5">
                                ...
                                <select name="linked_project_id" id="linked_project_id" class="kt-select w-full">
                                    ...
                                    @foreach($projects as $proj)
                                        <option value="{{ $proj->id }}" @selected((string) old('linked_project_id') === (string) $proj->id)>{{ $proj->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="famledger-form-field grid gap-1.5 {{ $projects->isEmpty() ? 'md:col-span-3' : 'md:col-span-2' }}">
                                ...
                                <select name="linked_property_id" id="linked_property_id" class="kt-select w-full">
                                    ...
                                    @foreach($properties as $prop)
                                        <option value="{{ $prop->id }}" @selected((string) old('linked_property_id') === (string) $prop->id)>{{ $prop->name }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="grid gap-1.5">
                        ...
                        <textarea name="notes" id="notes" rows="3" placeholder="{{ __('Optional notes') }}" class="kt-textarea resize-y max-w-3xl" aria-invalid="{{ $errors->has('notes') ? 'true' : 'false' }}">{{ old('notes') }}</textarea>
...
(function () {
    ...
    var oldIncomeCategoryId = @json((string) old('category_id', ''));
