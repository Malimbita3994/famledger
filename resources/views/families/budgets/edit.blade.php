                        ...
                        <div class="grow">
                            <select name="type" id="type" required class="kt-select">
                                @foreach (\App\Models\Budget::types() as $value => $label)
                                    <option value="{{ $value }}" {{ old('type', $budget->type) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grow">
                            <select name="wallet_ids[]" id="wallet_ids" class="kt-select" multiple size="4">
                                @foreach ($wallets as $w)
                                    <option value="{{ $w->id }}" {{ in_array($w->id, old('wallet_ids', $budget->wallets->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $w->name }} ({{ $w->currency_code }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grow">
                            <select name="category_ids[]" id="category_ids" class="kt-select" multiple size="4">
                                @foreach ($categories as $c)
                                    <option value="{{ $c->id }}" {{ in_array($c->id, old('category_ids', $budget->categories->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $c->name }}</option>
                                @endforeach
                            </select>
