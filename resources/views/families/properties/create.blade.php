...
<div class="kt-container-fixed px-4 sm:px-6 lg:px-8 py-6 lg:py-8 pb-12">
    ...
    <form action="{{ route('families.properties.store', $family) }}" method="POST" class="space-y-6">
        ...
        <div class="kt-card p-5 lg:p-7.5">
            <div class="mb-5">
                <h1 class="text-lg font-semibold text-mono">Add property</h1>
                <p class="text-sm text-muted-foreground mt-0.5">
                    Core attributes plus dynamic fields based on the selected category.
                </p>
            </div>
            ...
            <div class="grid gap-5 lg:gap-7.5">
                ...
                <div class="property-main-row">
                    ...
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <input id="name" type="text" name="name" class="kt-input" value="{{ old('name') }}" required>
                    ...
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <select id="ownership_type" name="ownership_type" class="kt-select">
                            ...
                            @foreach (['individual' => 'Individual', 'joint' => 'Joint', 'family_trust' => 'Family Trust'] as $key => $label)
                                <option value="{{ $key }}" @selected(old('ownership_type') === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <select id="owner_family_member_id" name="owner_family_member_id" class="kt-select">
                            ...
                            @foreach ($members as $member)
                                <option value="{{ $member->id }}" @selected(old('owner_family_member_id') == $member->id)>
                ...
                <div class="property-main-row">
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <input id="acquisition_date" type="date" name="acquisition_date" class="kt-input" value="{{ old('acquisition_date') }}">
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <select id="acquisition_method" name="acquisition_method" class="kt-select">
                            ...
                            @foreach (['purchase' => 'Purchase', 'inheritance' => 'Inheritance', 'gift' => 'Gift', 'exchange' => 'Exchange'] as $key => $label)
                                <option value="{{ $key }}" @selected(old('acquisition_method') === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        <label for="purchase_price" class="kt-form-label">Purchase price</label>
                        <input id="purchase_price" type="number" step="0.01" name="purchase_price" class="kt-input" value="{{ old('purchase_price') }}">
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        <label for="current_estimated_value" class="kt-form-label">Current estimated value</label>
                        <input id="current_estimated_value" type="number" step="0.01" name="current_estimated_value" class="kt-input" value="{{ old('current_estimated_value') }}">
                    </div>
                </div>
                ...
                <div class="property-main-row">
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <input id="country" type="text" name="country" class="kt-input" value="{{ old('country', $family->country) }}">
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        <label for="region_city" class="kt-form-label">Region / City</label>
                        <input id="region_city" type="text" name="region_city" class="kt-input" value="{{ old('region_city') }}">
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        <label for="address" class="kt-form-label">Physical address</label>
                        <input id="address" type="text" name="address" class="kt-input" value="{{ old('address') }}">
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        <label class="kt-form-label">GPS coordinates</label>
                        <div class="flex gap-2">
                            <input type="number" step="0.0000001" name="gps_lat" class="kt-input" placeholder="Lat" value="{{ old('gps_lat') }}">
                            <input type="number" step="0.0000001" name="gps_lng" class="kt-input" placeholder="Lng" value="{{ old('gps_lng') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="property-main-row">
                    <div class="property-main-col grid gap-1.5">
                        ...
                        <input id="title_number" type="text" name="title_number" class="kt-input" value="{{ old('title_number') }}">
                    </div>
                    <div class="property-main-col grid gap-1.5">
                        <label for="notes" class="kt-form-label">Notes</label>
                        <textarea id="notes" name="notes" rows="3" class="kt-textarea resize-y">{{ old('notes') }}</textarea>
                    </div>
                </div>
                        ...
                        @foreach ($attributes->chunk(4) as $chunk)
                            <div class="property-main-row mb-3">
                                @foreach ($chunk as $attr)
                                    @php $fieldName = 'attr_'.$attr->id; @endphp
                                    <div class="property-main-col grid gap-1.5">
                                        ...
                                        @switch($attr->data_type)
                                            @case('number')
                                                <input type="number" name="{{ $fieldName }}" class="kt-input" value="{{ old($fieldName) }}">
                                                @break
                                            @case('date')
                                                <input type="date" name="{{ $fieldName }}" class="kt-input" value="{{ old($fieldName) }}">
                                                @break
                                            @case('dropdown')
                                                <select name="{{ $fieldName }}" class="kt-select">
                                                    <option value="">Select</option>
                                                    @foreach ($attr->options as $opt)
                                                        <option value="{{ $opt->value }}" @selected(old($fieldName) == $opt->value)>{{ $opt->label ?? $opt->value }}</option>
                                                    @endforeach
                                                </select>
                                                @break
                                            @case('boolean')
                                                <select name="{{ $fieldName }}" class="kt-select">
                                                    <option value="">Select</option>
                                                    <option value="1" @selected(old($fieldName) === '1')>Yes</option>
                                                    <option value="0" @selected(old($fieldName) === '0')>No</option>
                                                </select>
                                                @break
                                            @default
                                                <input type="text" name="{{ $fieldName }}" class="kt-input" value="{{ old($fieldName) }}">
                                        @endswitch
                                    </div>
                                @endforeach
                            </div>
